...
// Windows strcasecmp/strncasecmp replacements
...
=======
            ...
            if (received <= 0) { printf("Connection closed by server\n"); break; }
            buffer[received]='\0';
            // Enhanced: Print green, capitalized output for SUCCESS/ERROR responses
            if (strstr(buffer, "SUCCESS:") || strstr(buffer, "ERROR:")) {
                // ANSI green: \033[1;32m ... \033[0m
                char *msg = buffer;
                // Find and print each SUCCESS/ERROR line in green, capitalized
                char *line = strtok(msg, "\n");
                while (line) {
                    if (strstr(line, "SUCCESS:") || strstr(line, "ERROR:")) {
                        // Capitalize line (Windows: _strupr)
...
=======
                if (!fp) {
                    ...
                    size_t zero=0; send(client_socket,(const char*)&zero,sizeof(size_t),0);
                } else {
                    fseek(fp,0,SEEK_END); size_t file_size = ftell(fp); fseek(fp,0,SEEK_SET);
                    send(client_socket,(const char*)&file_size,sizeof(size_t),0);
>>>>>>> 41fcdf1d0a6da8a0d825f87123969c4464751410
